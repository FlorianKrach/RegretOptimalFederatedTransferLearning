...
class Heston(Model):
    ...
    def var_drift_fct(self, x, v):
      v_positive = np.maximum(v, 0)
      return - self.speed * (v_positive - self.mean)

    def var_diffusion_fct(self, x, v):
      v_positive = np.maximum(v, 0)
      return self.volatility * np.sqrt(v_positive)

    def generate_paths_slow(self, start_X=None):
        paths = np.empty(
    ...
    def generate_paths(self, start_X=None):
        ...
        if start_X is not None:
          paths[:, :, 0] = start_X
            ...
            paths[:, :, 0] = self.spot
        var_paths[:, :, 0] = self.mean
        normal_numbers_1 = np.random.normal(
            0, 1, (self.nb_paths, self.nb_stocks, self.nb_dates))
        normal_numbers_2 = np.random.normal(
            0, 1, (self.nb_paths, self.nb_stocks, self.nb_dates))
        dW = normal_numbers_1 * np.sqrt(dt)
        dZ = (self.correlation * normal_numbers_1 + np.sqrt(
            1 - self.correlation ** 2) * normal_numbers_2) * np.sqrt(dt)
        for k in range(1, self.nb_dates + 1):
            var_paths[:, :, k] = (var_paths[:, :, k - 1] + self.var_drift_fct(
                paths[:, :, k - 1], var_paths[:, :, k - 1]) * dt +
                                  self.var_diffusion_fct(
                paths[:, :, k - 1], var_paths[:, :, k - 1]) * dZ[:, :, k - 1])

            paths[:, :, k] = (paths[:, :, k - 1] + self.drift_fct(
                paths[:, :, k - 1], None) * dt + self.diffusion_fct(
                paths[:, :, k - 1], None, var_paths[:, :, k-1]) * dW[:, :, k - 1])
        return paths, var_paths
